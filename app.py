...
st.markdown("### Giới thiệu ngắn")
st.write(
    "Ứng dụng này minh họa mô hình dự báo lãi suất khoản vay thế chấp nhà ở "
    "được xây dựng từ dữ liệu Freddie Mac. "
    "Bạn chỉ cần nhập các thông tin cơ bản của khoản vay, các thông tin nâng cao có thể để mặc định nếu chưa rõ."
)
...
with c1:
    ...
    original_ltv = st.number_input(
        ...
        min_value=0.0, max_value=120.0, value=80.0,
...
with c2:
    ...
    original_loan_term = st.number_input(
        ...
        min_value=12, max_value=480, value=360,
...
with st.expander("2. Thông tin nâng cao (có thể giữ mặc định nếu chưa rõ)", expanded=False):
    ...
    with c3:
        original_cltv = st.number_input(
            "Tỷ lệ cho vay gộp (%)",
            min_value=0.0, max_value=150.0, value=80.0,
    ...
    with c4:
        ...
        property_valuation_method = st.text_input(
            "Phương pháp định giá tài sản",
            value="A",
            help="Nếu chưa rõ, giữ mặc định."
        )
